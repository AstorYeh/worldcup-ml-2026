...
| 項目 | 內容 |
|------|------|
| **資料規模** | 49,328 場國際比賽（1872-2026）+ 67,894 筆 FIFA 排名 + 240 球員陣容 |
...
| **驗證準確率** | 51.7%（vs 隨機猜測 33.3%，提升 +18.4%） |
...
│     賠率每日由 GitHub Action 自動更新                │
└──────────────────────────────────────────────────────┘
```

> **市場校準（L5）**：把博彩冠軍盤口賠率去除莊家抽水（de-vig）轉成「市場共識機率」，
> 與模型加權融合。權重 α 隨開賽逼近上升（離賽越近市場越準）。
> 賠率快照 `market_odds.py` 由 `.github/workflows/update-odds.yml` 每日凌晨自動重抓更新
> （需設定 `ODDS_API_KEY` repo secret；未設定則沿用快照，app 照常運作）。
...
| 分頁 | 內容 |
|------|------|
| 📊 **專題總覽** | Hero 圖、資料規模、12 組分組卡、四層模型架構（垂直階層流程圖） |
...
| 🏅 **奪冠預測** | Monte Carlo Top 20 奪冠機率、進決賽機率、小組第一機率 |
...
| 訓練資料 | 驗證集 | 準確率 |
|---------|--------|--------|
| 1990-2009 | WC 2010 | 52.0% |
| 1990-2013 | WC 2014 | 50.0% |
| 1990-2017 | WC 2018 | 50.0% |
| 1990-2021 | WC 2022 | 54.7% |
| **平均** | — | **51.7%** |

→ 比隨機猜測 33.3% 高出 **+18.4 個百分點**
